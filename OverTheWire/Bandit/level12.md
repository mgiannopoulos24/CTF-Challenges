Use the `ls` command to see the contents of `bandit12`.
```console
bandit12@bandit:~$ ls
data.txt
```
As shown, there is only one file, `data.txt`.  

Then, use the `cat` command to read it. As it is a hexdump of a file that was repeatedly compressed using different methods, the goal is to retrieve the original file from it.  

You should now create a temporary directory using the command `mktemp -d` (using it will show the name of the temporary directory for you to keep), where you will copy the file using the `cp` command. Finally, go to that directory with the command `cd`. This will be your working directory.  
